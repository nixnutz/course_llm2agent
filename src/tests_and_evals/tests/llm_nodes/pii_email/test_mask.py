...
from src.llm_nodes.pii_email.mask import demask_pii_emails, mask_pii_emails
...
def test_dedupe_same_canonical_shares_placeholder():
    ...
    assert result.emails == ["ulf.wendel@phpdoc.de"]
...
def test_canonical_key_is_strip_lower():
    ...
    assert result.emails == ["foo@bar.com"]
    assert result.occurrences[0].email == "foo@bar.com"
...
def test_span_not_found_is_skipped_and_raw_discarded():
    ...
    assert result.text == inp
    assert result.emails == []  # raw discarded, not added
    assert len(result.occurrences) == 1
...
def test_restore_roundtrip_replaces_all_placeholders():
    ...
    restored = demask_pii_emails(result.text, result)
...
@pytest.mark.unit
def test_demask_leaves_none_email_placeholders():
    inp = "Reach broken add at nowhere today"
    result = mask_pii_emails(inp, [{"span": "broken add at nowhere", "raw": "not-an-email"}])
    ...
    assert placeholder in restored
    assert "broken add at nowhere" not in restored
