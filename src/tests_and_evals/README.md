...
- `evals/`: manual golden-set/model-comparison checks
  - `evals/eval_collector.py`: shared per-session result store (`get_eval_collector`, xdist merge helpers)
  - `evals/conftest.py`: eval-tree pytest hooks (reset collector, SHOULD gate); node tests live under `evals/llm_nodes/<node>/`
