...
- `SBASH_SESSION_NETWORK_NAME` — inner Docker bridge for session containers (default `sbash_sessions`; ICC disabled at network create).
...
The smoke test covers the API without LangGraph: health, session start, same-session state, non-zero exit, timeout, over-default timeout rejection, script-size rejection, stdout/stderr output limits, dummy and omitted correlation metadata, absence of `GET /sessions`, session-side negative checks (primary: `backend_core` bind IP must be unreachable from sessions; supplementary: session-bridge gateway), session-to-host blocks on non-API ports, session-to-session ICC block, optional outbound internet probe, and cleanup.
...
- **Podman Compose:** the host stack on Podman is untested; inner session network creation still uses the inner `docker` CLI — verification today is Docker Compose only.
...
- **No extra `cap_add` for iptables (lab assumption A1):** the Sysbox **system container**
  ...
  **before** uvicorn starts (`run-api.sh` order: bind resolve → session network ensure → iptables → API). The HTTP
...
| **HTTP exposure** | Internal Compose network; binds `backend_core` IP only; `iptables` INPUT DROP from session-bridge CIDR (all ports); no v1 auth; no host port by default | Do not expose the API outside the lab network without a separate security review |
...
| **Network** | Sessions on dedicated inner bridge `sbash_sessions` (ICC off); outbound internet allowed; Sandbox API and host netns blocked from sessions (bind + full CIDR `iptables` DROP); session → other `backend_core` services (e.g. `postgres`, `litellm`) via NAT remains possible — accepted lab limit | Document clearly: not production-safe for untrusted code |
...
If `sysbox-bash-api.service` fails at startup with `could not resolve CIDR for sbash_sessions`, inner
Docker may not have created the session bridge yet. The firewall script retries for up to 30 seconds;
