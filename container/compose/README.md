...
- Network `backend_core`: `caddy` + backend services (`litellm`, `phoenix`, `postgres`, `ollama`, init jobs)
...
- LiteLLM: `https://caddy:${LITELLM_PORT:-4000}`
- Phoenix UI: `https://caddy:${PHOENIX_UI_TLS_PORT:-6006}`

Direct backend access from `dev` (for example `http://litellm:${LITELLM_INTERNAL_PORT}`) is intentionally blocked by network segmentation.
