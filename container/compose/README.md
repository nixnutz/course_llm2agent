...
**Exception — `sysbox_bash`:** `dev` also joins `backend_core` and receives `SBASH_BASE_URL=http://sysbox_bash:${SBASH_PORT:-8080}` for the Bash sandbox HTTP API (Slice 1: `GET /health` only). This is direct HTTP on the Compose network, not via Caddy. Container env uses `SBASH_*` names (not `SYSBOX_*`) because Sysbox reserves `SYSBOX_` for runtime configuration.
